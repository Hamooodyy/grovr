...
/**
 * Extracts a normalized size in oz from a product name string.
 * Handles "fl oz", "oz", and "lb" (1 lb = 16 oz).
 */
...
/**
 * Returns a bonus score [−0.5, +1] based on how well a product's size
 * matches the user's S / M / L preference.
 *   S  →  < 20 oz   (pint, single-serve, small carton)
 *   M  →  20–79 oz  (quart, standard package)
 *   L  →  ≥ 80 oz   (half-gallon, gallon, family size, 5 lb+)
 */
function sizeScore(productName: string, size?: import("./types").ItemSize): number {
  ...
  return -0.5; // wrong size — deprioritize
}
